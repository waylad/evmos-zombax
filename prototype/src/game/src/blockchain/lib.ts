...
export const getCars = async () => {
  ...
  ownedCarsIds.forEach(async (ownedCarsId) => {
    ...
    state.ownedCars.push({
      ...
      carCode: carMeta[3].replace('https://battlenevs.io/assets/cars/', '').replace('.json', ''),
  ...
  onSaleCarsIds.forEach(async (onSaleCar) => {
    state.onSaleCars.push({
      ...
      carCode: onSaleCar[3].replace('https://battlenevs.io/assets/cars/', '').replace('.json', ''),
...
export const upgradeCar = async (carToken: CarToken) => {
  const receipt = await carsContractWithSigner.updateTokenUri(
    carToken.tokenId,
    `https://battlenevs.io/assets/cars/${carToken.carCode}.json`,
  )
  const tx = await receipt.wait()
  console.log(tx)
}